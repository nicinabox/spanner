...
export const vehicleAPIPath = (vehicleId: API.RecordID, share?: boolean) =>
    `/api/vehicles/${vehicleId}${share ? '/share' : ''}`;
...
export async function fetchVehicle(
    ...
    const { data } = await api.get<API.Vehicle>(
        vehicleAPIPath(vehicleId, share),
    );
    return data;
}
...
export async function updateVehicle(
    api: AxiosInstance,
    params: MutateParams<VehicleParams>,
) {
    const { id, ...updateParams } = params;
    const { data } = await api.put<API.Vehicle>(
        vehicleAPIPath(id),
        updateParams,
    );
    return data;
...
export async function importRecords(
    api: AxiosInstance,
    vehicleId: API.RecordID,
    params: { importFile: File; fuelly: boolean },
) {
    ...
    const { data } = await api.post(
        `${vehicleAPIPath(vehicleId)}/import`,
        body,
    );
    return data;
}
