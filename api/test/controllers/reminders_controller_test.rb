...
class RemindersControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @reminder = @vehicle.reminders.create!(notes: 'Oil change', date: Time.zone.today + 7, mileage: 1000,
                                           reminder_type: 'date')
    @headers = {
  ...
  test 'create a reminder' do
    assert_difference('@vehicle.reminders.count', 1) do
      post vehicle_reminders_url(@vehicle),
           params: { reminder: { notes: 'Tire rotation', date: Time.zone.today + 14,
                                 mileage: 2000, reminder_type: 'date' } }, headers: @headers
    end
    assert_response :success
    body = JSON.parse(@response.body)
  ...
  test 'estimate date' do
    get vehicle_reminders_estimate_date_url(@vehicle),
        params: { reminder: { notes: 'Oil change', mileage: 5000, reminder_type: 'mileage' } }, headers: @headers
    assert_response :success
    body = JSON.parse(@response.body)
    assert body.key?('reminder_date')
  end
end
