...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onclick={onClose}>
	<!-- svelte-ignore a11y_click_events_have_key_events a11y_no_static_element_interactions -->
	<div class="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[80vh] overflow-y-auto" onclick={(e) => e.stopPropagation()}>
		<div class="flex items-center justify-between p-4 border-b border-ink-200">
			<h2 class="text-lg font-medium">Suggest Schedules</h2>
			<button onclick={onClose} class="text-ink-400 hover:text-ink-700">
				<X size={20} />
			</button>
		</div>
		...
		<form method="POST" action="?/suggest" use:enhance>
			<div class="p-4">
				...
				{:else if !selectedType}
					<p class="text-ink-500 mb-4">Pick a vehicle type to see suggested schedules:</p>
